  ...
  return (
    <div className="flex min-w-0 flex-1 flex-col overflow-hidden rounded-[14px] border-2 border-navy bg-card">
      <div className="flex flex-col gap-[10px] bg-green px-5 py-[18px] sm:px-[26px] sm:py-[20px]">
        <span className="inline-block self-start rounded-full bg-cream px-4 py-2 text-[12px] font-extrabold leading-[1.35] tracking-[0.07em] text-navy sm:tracking-[0.08em]">
          {badge}
        </span>
        <span className="font-display text-[25px] leading-[1.1] tracking-[0.01em] text-cream sm:text-[24px]">{title}</span>
      </div>
      <div className="flex flex-1 flex-col gap-4 p-5 sm:p-[26px]">
        <p className="m-0 max-w-[46ch] text-[18px] leading-[1.5] text-ink sm:text-[17px] sm:leading-[1.55]">{body}</p>
        <a
          href="#signup"
    ...
    <section id="prompts" className="border-b-2 border-navy px-5 py-11 sm:px-10 sm:py-[60px]">
      <div className="mx-auto flex max-w-[1060px] flex-col gap-6">
        ...
        <div className="flex flex-col items-center justify-center gap-4 pt-3 text-center sm:flex-row sm:gap-[18px] sm:pt-4">
          <div className="max-w-[24ch] text-balance font-display text-[29px] leading-[1.12] tracking-[0.01em] text-navy sm:max-w-none sm:text-[26px] sm:leading-[1.15]">
            That&apos;s just a peek. Get the complete prompts free.
          </div>
          <a
            href="#signup"
            className="inline-flex h-[52px] items-center whitespace-nowrap rounded-[26px] border-2 border-navy bg-primary px-6 font-display text-[18px] tracking-[0.02em] text-primary-foreground transition-colors hover:bg-primary-hover hover:text-primary-foreground"
          >
            GET THE PROMPTS
          </a>
        </div>
